    ...
    <div class="block">
        ...
        <div id="tablewidget" class="block-body collapse in">
            <table class="table">
              ...
              <tbody id="tbody">
              <%
              		int pages_=(pages-1)*5;
              		for(int i=0;i<list.size();i++){
              			MusicInfo mu=(MusicInfo)list.get(i);
              			String Del_url="../Del_Music_Servlet?music_id="+mu.getId();
              		//	session.setAttribute(String.valueOf(mu.getId()), mu);
              %>
                <tr>
                  <td><%=pages_+i+1 %></td>
                  <td><%=mu.getMusic_name() %></td>
                  <td><%=mu.getMusic_song() %></td>
                  <td><a href=<%=Del_url %> class="deleteBtn">删除</a></td>     
                </tr>
               <%
              		}
               %>
            ...
            <ul class="list-inline pageSort pagination">
	        	<li><a href="musicList.jsp?page=1">Prev</a></li>
           		 <%
           			 for(int i=1;i<=count;i++){
						String addr="musicList.jsp?page="+i;
           		 %>
           			 <li><a href=<%=addr %>><%=i %></a></li>
            	<%
           			 }
             		String next="musicList.jsp?page="+count;
            	%>
